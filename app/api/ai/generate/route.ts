...
export async function POST(request: Request) {
  ...
  const result = await sql`
    SELECT u.id AS user_id,ais.provider,ais.model,ais.api_key_ciphertext,ais.api_key_iv,
      ...
      cv.text_ciphertext AS cv_text_ciphertext,cv.text_iv AS cv_text_iv,
  ...
  if (!row.headline && !(row.skills as string[] | null)?.length && !row.cv_text_ciphertext) return NextResponse.json({ error: "Complète ton profil ou importe ton CV pour obtenir un texte fiable", needsProfile: true }, { status: 409 });
